...
enum NamedColor {
  ...
  /**
   * https://www.it-swarm-ja.com/ja/algorithm/色の類似性をチェックするアルゴリズム/971550257/
   */
  public static final ColorComparator TRComparator = new ColorComparator() {
    public double distance(int r1, int g1, int b1, int r2, int g2, int b2) {
  ...
  public static NamedColor getApproximateColor(int rgb) {
    return getApproximateColor(rgb, null);
  }

  /**
   * comp を使って, 定義されている中でもっとも rgb に近い色を返す.<br>
   * comp が null の場合はデフォルトを使用する.
   * @param rgb RGBカラー
   * @param comp 比較オブジェクト
   * @return NamedColor オブジェクト
   */
  public static NamedColor getApproximateColor(int rgb, ColorComparator comp) {
    if(comp == null) comp = NamedColor.comparator;
    ...
    for(NamedColor c : values()) {
      double value = comp.distance(c.r, c.g, c.b, r, g, b);
      if(min > value) {
        min = value;
        color = c;
      }
